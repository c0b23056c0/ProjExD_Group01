...
class Start:
    def __init__(self):
        ...
        self.txt = self.fonto.render("Start", True, (255,0,0)) #start文字
    ...
    def update(self, screen: pg.Surface):
        ...
        screen.blit(self.txt,[600/2 - 120, 900/2 - 70])
        screen.blit(self.image1,[100, 380])
        screen.blit(self.image2,[400, 380])
...
def main(stage_num):
    ...
    while True:
        ...
        if pg.mouse.get_pressed()[0]: # マウスの処理
            mouse_x, mouse_y = pg.mouse.get_pos()
            if (WIDTH / 2 - 60 <= mouse_x <= WIDTH / 2 + 60) and (HEIGHT / 2 - 60 <= mouse_y <= HEIGHT / 2 + 60):
                break
